...
Prepped는 시니어 사용자가 자주 먹는 메뉴 조합을 모바일에서 QR로 저장하고,
키오스크에서 해당 QR을 스캔해 주문 선택 단계를 줄이는 PWA 및 키오스크 웹앱이다.
...
- QR payload는 `store={menuId,menuId}` 형식을 유지하고, 여러 매장은 세미콜론으로 구분한다.
- 모바일 앱의 기기 로컬 상태는 브라우저 저장소에 보관하며, 서버 동기화로 오해하지 않도록 한다.
- `/`는 모바일 QR 생성 PWA, `/kiosk`는 노트북 카메라 기반 키오스크 스캐너 경로로 유지한다.
- 백엔드 API가 확정되기 전에는 QR 원문 및 메뉴 ID 파싱을 계약 경계로 유지한다.
- 사용자 인터페이스 변경 후 `npm run build`로 두 경로의 빌드를 검증한다.
...
- [`.openai/hosting.json`](.openai/hosting.json) — Sites 배포 프로젝트와 리소스 바인딩
