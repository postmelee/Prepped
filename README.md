...
| 경계 | 현재 책임 |
|---|---|
| 모바일 PWA | mock 메뉴 선택, 매장별 저장 조합과 QR 포함 상태 관리, payload 직렬화와 QR 이미지 생성 |
...
메뉴 조합과 QR 포함 여부는 브라우저 `localStorage`의 `onemeal-menu-v1`에만 저장됩니다. 현재는 사용자 계정, 서버 백업 또는 다른 기기와의 동기화를 제공하지 않습니다.
...
| `npm test` | 빌드 후 모바일·키오스크 렌더링과 PWA·QR 계약 회귀 검사 |
