...
이 프로젝트는 결과물뿐 아니라, 해커톤이 중요하게 보는 “Codex와 어떻게 함께 만들었는가”의 과정도 남기고자 했습니다. 이를 위해 Hyper-Waterfall 방법론을 적용하여 GitHub Issue, 작업 계획, 단계별 구현과 검증, 완료 보고서, Pull Request를 하나의 추적 가능한 흐름으로 관리했습니다.

`mydocs/`에는 Codex가 수행한 분석과 구현뿐 아니라, 사람이 방향을 결정하고 계획을 승인하며 결과를 검토한 과정도 함께 기록되어 있습니다. 완성된 코드만 보여주는 데 그치지 않고, Codex와의 협업 과정과 시행착오, 판단 근거를 누구나 확인할 수 있는 개발 기록으로 남기는 것이 이 방식을 사용한 이유입니다.
