...
- 최종 계약·보안 검증: API 조회·완료 경로, 기존 `mcdonald={...}` 호환성, `develop/backend → main` 배포 규칙, GitHub OIDC의 `main` subject 조건을 문서에서 확인했다.
- 미해결 표식·실제 AWS 액세스 키 형식 검증: `TODO`·`TBD`와 `AKIA...` 형식이 공식 문서에 없다.
...
- 25달러 크레딧의 적용 범위·만료·잔액은 계정 소유자가 Billing 콘솔에서 직접 확인해야 한다.
